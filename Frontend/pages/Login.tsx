...
const Login: React.FC<LoginProps> = ({ onLogin, onBack }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Truyền cả user lẫn token lên App.tsx để App tự lưu token trước khi navigate
      onLogin(
        {
          ...userData,
        ...
        <div className="md:w-1/2 p-12 lg:p-16">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-[#ff5c62] text-white py-4 rounded-2xl font-bold text-lg hover:bg-[#ee4b51] transition-all shadow-xl shadow-red-200 flex items-center justify-center gap-3 group disabled:opacity-70 disabled:cursor-not-allowed"
            >
